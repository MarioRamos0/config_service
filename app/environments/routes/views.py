...
class EnvironmentResponse(Environment):
    pass
...
@router.get("/", response_model=PaginatedEnvironmentResponse, summary="List Environments", 
...
def list_environments(
    ...
    try:
        ...
        count_statement = select(Environment)
        total_count = len(session.exec(count_statement).all())
        statement = select(Environment).offset(offset).limit(page_size)
        environments = session.exec(statement).all()
        next_url = None
        previous_url = None
...
@router.post("/", response_model=EnvironmentResponse, summary="Create Environment",
          description="Create a new environment with the provided details.", status_code=status.HTTP_201_CREATED)
def create_environment(
    ...
    try:
        statement = select(Environment).where(Environment.name == environment.name)
        existing_env = session.exec(statement).first()
        if existing_env:
...
@router.get("/{env_name}/", response_model=EnvironmentResponse, summary="Get Environment",
...
@router.put("/{env_name}/", response_model=EnvironmentResponse, summary="Update Environment",
...
def update_environment(
    ...
    environment_update: Environment = None,
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_active_user)
):
    try:
        ...
        if not environment:
            raise HTTPException(status_code=404, detail="Environment not found")
        if environment_update.description is not None:
...
#Actualizar un environment parcialmente
@router.patch("/{env_name}/", response_model=EnvironmentResponse, summary="Partially Update Environment",
